...
All tools use `Codex` or `codex` CLI as the LLM backend (no API keys needed):

```bash
# autoevolve
python3 autoevolve.py seed.py evolve.md -g 5 -n 3 -b "python3 bench.py {file}" -e Codex

# autoship
cd autoship && python3 autoship.py example.md -e codex

# spawn (parallel mode)
cd spawn && python3 spawn.py example.md -e Codex
...
- All three tools call LLMs via subprocess (`Codex -p` or `codex exec --full-auto`), not via API SDK
- The `Codex` env var must be unset to allow nesting Codex calls (`env.pop("Codex", None)`)
- Codex requires a git repo in the working directory; scripts auto-run `git init` when needed
- `--dangerously-skip-permissions` (Codex) and `--full-auto` (codex) enable autonomous operation
